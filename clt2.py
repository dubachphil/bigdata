...
class MRAvgPairwiseLines(MRJob):
    ...
    def input_combiner(self,key,values):
       values = list(values)
       if len(values) == 3:
...
############################################################################
    ...
    def output_combiner(self,key,values):
    ...
    def steps(self):
       return [
               MRStep(mapper=self.input_mapper, combiner=self.input_combiner, reducer=self.input_reducer),
               MRStep(mapper=self.output_mapper, combiner=self.output_combiner, reducer=self.output_reducer)
              ]
